# 👋 Hi, I'm Parth (Ambivert01)

Backend & Full-Stack Developer | Production-Grade Systems | CS Engineer  
Focused on building **scalable, secure, real-world applications**

---

## 🧠 About Me
- 🎓 Computer Science Engineering student  
- 🔧 Backend-focused developer with system-level thinking  
- 🚀 Built **production-ready systems with documentation**, not tutorials  
- 🧩 Comfortable with frontend → backend → database → Linux  
- 📚 Strengthening DSA, core CS fundamentals & technical skills continuously  

---

## 🛠️ Technical Skills

### 💻 Programming Languages
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-323330?style=for-the-badge&logo=javascript)

---

### 🌐 Frontend
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react)

---

### ⚙️ Backend & Databases
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white)
![Mongoose](https://img.shields.io/badge/Mongoose-880000?style=for-the-badge)
![SQL](https://img.shields.io/badge/SQL-003B57?style=for-the-badge&logo=mysql&logoColor=white)

---

### 🧰 Tools & Systems
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![REST](https://img.shields.io/badge/REST%20API-005571?style=for-the-badge)
![JWT](https://img.shields.io/badge/JWT-000000?style=for-the-badge)
![MVC](https://img.shields.io/badge/MVC-6A1B9A?style=for-the-badge)

---

## 📊 GitHub Overview

### 🔹 Contribution Activity Graph
![GitHub Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=Ambivert01&theme=react-dark&hide_border=true)

---

### 🔹 GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Ambivert01&show_icons=true&theme=dark&hide_border=true&count_private=true)

---

### 🔹 Most Used Languages
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Ambivert01&layout=compact&theme=dark&hide_border=true)

---

### 🔹 Contribution Streak
![GitHub Streak](https://streak-stats.demolab.com?user=Ambivert01&theme=dark&hide_border=true)

---

### 🔹 GitHub Trophies
![Trophies](https://github-profile-trophy.vercel.app/?username=Ambivert01&theme=darkhub&no-frame=true)

---

## 🚀 Production-Level Projects

### 🔹 AidFlow
- Secure backend architecture
- JWT authentication & RBAC
- RESTful APIs
- MongoDB + Mongoose modeling
- Production-ready error handling

### 🔹 Behaviour Guardian
- Real-world backend application
- Express-based scalable architecture
- Secure API design
- Data validation & sanitization

### 🔹 Professional Backend Project
- Modular folder structure
- Clean API responses
- Middleware-driven logic
- Designed for deployment, not demos

👉 *Best repositories are pinned below ⬇️*

---

## 🎯 Focus Areas
- Backend Engineering
- Scalable REST APIs
- Clean architecture
- System-oriented development
- Production-grade software

---

⭐ *Open to collaboration on meaningful backend & full-stack projects*
